...
namespace UnitTestProject1.helper
{
    ...
    {
        private static IDictionary _parentTestMap = new Dictionary<string,ExtentTest>();
        ...
        public static ExtentTest CreateMethod(string parentName, string testName, string description = null)
        {
            lock (_synclock)
            {
                ...
                if (!_parentTestMap.Contains(parentName))
                {
                ...
                {
                    parentTest = _parentTestMap[parentName] as ExtentTest;
                }
        ...
        public static ExtentReports ExtentReportInstance()
        {
            lock (_synclock)
            {
                if (_extent==null)
                {
                    _extent = ExtentService.Instance; 
                }
                return _extent; 
            }
        }
